...
static mxArray* do_forward(const mxArray** const prhs) {
  const mxArray* const bottom = prhs[0];
  string from = mxArrayToString(prhs[1]);
  string to = mxArrayToString(prhs[2]);
  string name = mxArrayToString(prhs[3]);
  ...
  int start = net_->layer_names_index().at(from);
  int end = net_->layer_names_index().at(to);
  //mexPrintf("forward: start=%d, end=%d, total=%d\n", start, end, net_->layers().size());
  //net_->ForwardPrefilled();
  net_->ForwardFromTo(start,end);
  vector<shared_ptr<Blob<float> > > output_blobs;
  output_blobs.push_back(net_->blob_by_name(name));
...
static mxArray* do_backward(const mxArray** const prhs) {
  const mxArray* const top_diff = prhs[0];
  const string from  = mxArrayToString(prhs[1]);
  const string to = mxArrayToString(prhs[2]);
  ...
  int start = net_->layer_names_index().at(from);
  int end = net_->layer_names_index().at(to);
  //mexPrintf("backward: start=%d, end=%d\n", start, end);
...
static void forward(MEX_ARGS) {
  if (nrhs != 4) {
    ostringstream error_msg;
    error_msg << "Expected 4 argument, got " << nrhs;
    mex_error(error_msg.str());
  }

  plhs[0] = do_forward(prhs);
}

static void backward(MEX_ARGS) {
  if (nrhs != 3) {
    ostringstream error_msg;
    error_msg << "Expected 3 argument, got " << nrhs;
    mex_error(error_msg.str());
  }

  plhs[0] = do_backward(prhs);
}
...
static void update(MEX_ARGS) {
    ...
    const vector<shared_ptr<Blob<float> > >& history = net_->history();
    const vector<float>& net_params_lr = net_->params_lr();
    const vector<float>& net_params_weight_decay = net_->params_weight_decay();

    float rate = mxGetScalar(prhs[0]);
    float momentum = 0.95;
    ...
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
        ...
        caffe_gpu_axpby(net_params[param_id]->count(), local_rate,
                ...
                history[param_id]->mutable_gpu_data());
...
static void presolve(MEX_ARGS) {
    const vector<shared_ptr<Blob<float> > >& net_params = net_->params();
    vector<shared_ptr<Blob<float> > >& history = net_->history();
    history.clear();
    for (int i = 0; i<net_params.size(); ++i) {
        const vector<int>& shape = net_params[i]->shape();
        history.push_back(shared_ptr<Blob<float> >(new Blob<float>(shape)));
    }
}
